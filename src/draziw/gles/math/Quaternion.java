...
public class Quaternion {
	 ...
	 * @param angle The angle in degrees
	 * @return This quaternion for chaining. */
	public Quaternion set (float[] axis, float angle) {
		return setFromAxis(axis[0], axis[1], axis[2], angle);
	}
	...
	public Quaternion setEulerAngles (float yaw, float pitch, float roll) {
		return setEulerAnglesRad(yaw * MyMath.degreesToRadians, pitch * MyMath.degreesToRadians, roll
			* MyMath.degreesToRadians);
	}
	...
	public float getRoll() {
		return getRollRad() * MyMath.radiansToDegrees;
	}
	...
	public float getPitchRad() {
		final int pole = getGimbalPole();
		return pole == 0 ? (float)Math.asin(MyMath.clamp(2f*(w*x-z*y), -1f, 1f)) : (float)pole * MyMath.PI * 0.5f;
	}

	/** Get the pitch euler angle in degrees, which is the rotation around the x axis. Requires that this quaternion is normalized. 
	 * @return the rotation around the x axis in degrees (between -90 and +90) */
	public float getPitch() {
		return getPitchRad() * MyMath.radiansToDegrees;
	}
	...
	public float getYaw() {
		return getYawRad() * MyMath.radiansToDegrees;
	}
	...
	public Quaternion nor () {
		float len = len2();
		if (len != 0.f && !MyMath.isEqual(len, 1f)) {
...
// method
	...
	public void toMatrix (final float[] matrix) {
		...
		matrix[MyMatrix.M00] = 1 - 2 * (yy + zz);
		matrix[MyMatrix.M01] = 2 * (xy - zw);
		matrix[MyMatrix.M02] = 2 * (xz + yw);
		matrix[MyMatrix.M03] = 0;
		matrix[MyMatrix.M10] = 2 * (xy + zw);
		matrix[MyMatrix.M11] = 1 - 2 * (xx + zz);
		matrix[MyMatrix.M12] = 2 * (yz - xw);
		matrix[MyMatrix.M13] = 0;
		matrix[MyMatrix.M20] = 2 * (xz - yw);
		matrix[MyMatrix.M21] = 2 * (yz + xw);
		matrix[MyMatrix.M22] = 1 - 2 * (xx + yy);
		matrix[MyMatrix.M23] = 0;
		matrix[MyMatrix.M30] = 0;
		matrix[MyMatrix.M31] = 0;
		matrix[MyMatrix.M32] = 0;
		matrix[MyMatrix.M33] = 1;
	}
	...
	public boolean isIdentity () {
		return MyMath.isZero(x) && MyMath.isZero(y) && MyMath.isZero(z) && MyMath.isEqual(w, 1f);
	}

	/** @return If this quaternion is an identity Quaternion */
	public boolean isIdentity (final float tolerance) {
		return MyMath.isZero(x, tolerance) && MyMath.isZero(y, tolerance) && MyMath.isZero(z, tolerance)
			&& MyMath.isEqual(w, 1f, tolerance);
	}
	 ...
	 * @return This quaternion for chaining. */
	public Quaternion setFromAxis (float[] axis, final float degrees) {
		return setFromAxis(axis[0], axis[1], axis[2], degrees);
	}
	 ...
	 * @return This quaternion for chaining. */
	public Quaternion setFromAxisRad (float[] axis, final float radians) {
		return setFromAxisRad(axis[0], axis[1], axis[2], radians);
	}
	...
	public Quaternion setFromAxis (final float x, final float y, final float z, final float degrees) {
		return setFromAxisRad(x, y, z, degrees * MyMath.degreesToRadians);
	}
	...
	public Quaternion setFromAxisRad (final float x, final float y, final float z, final float radians) {
		float d = MyMath.vector3Len(x, y, z);
		if (d == 0f) return idt();
		d = 1f / d;
		float l_ang = radians < 0 ? MyMath.PI2 - (-radians % MyMath.PI2) : radians % MyMath.PI2;
	...
	public Quaternion setFromMatrix (boolean normalizeAxes, float[] matrix) {
		return setFromAxes(normalizeAxes, matrix[MyMatrix.M00], matrix[MyMatrix.M01], matrix[MyMatrix.M02],
			matrix[MyMatrix.M10], matrix[MyMatrix.M11], matrix[MyMatrix.M12], matrix[MyMatrix.M20],
			matrix[MyMatrix.M21], matrix[MyMatrix.M22]);
	}

	/** Sets the Quaternion from the given rotation matrix, which must not contain scaling. */
	public Quaternion setFromMatrix (float[] matrix) {
	...
	public Quaternion setFromAxes (boolean normalizeAxes, float xx, float xy, float xz, float yx, float yy, float yz, float zx,
		float zy, float zz) {
		if (normalizeAxes) {
			final float lx = 1f / MyMath.vector3Len(xx, xy, xz);
			final float ly = 1f / MyMath.vector3Len(yx, yy, yz);
			final float lz = 1f / MyMath.vector3Len(zx, zy, zz);
			xx *= lx;
			xy *= lx;
			xz *= lx;
			yx *= ly;
	 ...
	 * @return This quaternion for chaining */
	public Quaternion setFromCross (final float[] v1, final float[] v2) {
		final float dot = MyMath.clamp(MyMath.vector3Dot(v1,v2), -1f, 1f);
		final float angle = (float)Math.acos(dot);
		return setFromAxisRad(v1[1] * v2[2] - v1[2] * v2[1], v1[2] * v2[0] - v1[0] * v2[2], v1[0] * v2[1] - v1[1] * v2[0], angle);
	}
	...
	public Quaternion setFromCross (final float x1, final float y1, final float z1, final float x2, final float y2, final float z2) {
		final float dot = MyMath.clamp(MyMath.vector3Dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
		final float angle = (float)Math.acos(dot);
	...
	public float getAxisAngle (float[] axis) {
		return getAxisAngleRad(axis) * MyMath.radiansToDegrees;
	}
	 ...
	 * @see <a href="http://en.wikipedia.org/wiki/Axis%E2%80%93angle_representation">wikipedia</a>
	 * @see <a href="http://www.euclideanspace.com/maths/geometry/rotations/conversions/quaternionToAngle">calculation</a> */
	public float getAxisAngleRad (float[] axis) {
		...
		if (s < MyMath.FLOAT_ROUNDING_ERROR) { // test to avoid divide by zero, s is always positive due to sqrt
			// if s close to zero then direction of axis not important
			axis[0] = this.x; // if it is important that axis is normalised then replace with x=1; y=z=0;
			axis[1] = this.y;
			axis[2] = this.z;
		} else {
			axis[0] = (float)(this.x / s); // normalise axis
			axis[1] = (float)(this.y / s);
			axis[2] = (float)(this.z / s);
		}
	...
	public float getAngle () {
		return getAngleRad() * MyMath.radiansToDegrees;
	}
	...
	public void getSwingTwist (final float axisX, final float axisY, final float axisZ, final Quaternion swing,
		...
		final float d = MyMath.vector3Dot(this.x, this.y, this.z, axisX, axisY, axisZ);
	 ...
	 * @param twist will receive the twist rotation: the rotation around the specified axis
	 * @see <a href="http://www.euclideanspace.com/maths/geometry/rotations/for/decomposition">calculation</a> */
	public void getSwingTwist (final float[] axis, final Quaternion swing, final Quaternion twist) {
		getSwingTwist(axis[0], axis[1], axis[2], swing, twist);
	}
	...
	public float getAngleAroundRad (final float axisX, final float axisY, final float axisZ) {
		...
		return MyMath.isZero(l2) ? 0f : (float)(2.0 * Math.acos(MyMath.clamp((float) (this.w / Math.sqrt(l2)), -1f, 1f)));
	}

	/** Get the angle in radians of the rotation around the specified axis. The axis must be normalized.
	 * @param axis the normalized axis for which to get the angle
	 * @return the angle in radians of the rotation around the specified axis */
	public float getAngleAroundRad (final float[] axis) {
		return getAngleAroundRad(axis[0], axis[0], axis[0]);
	}
	...
	public float getAngleAround (final float axisX, final float axisY, final float axisZ) {
		return getAngleAroundRad(axisX, axisY, axisZ) * MyMath.radiansToDegrees;
	}

	/** Get the angle in degrees of the rotation around the specified axis. The axis must be normalized.
	 * @param axis the normalized axis for which to get the angle
	 * @return the angle in degrees of the rotation around the specified axis */
	public float getAngleAround (final float[] axis) {
		return getAngleAround(axis[0], axis[1], axis[2]);
	}
