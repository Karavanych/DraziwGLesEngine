...
public class Old_Custom3D extends GLESObject {
	...
	public Old_Custom3D(Texture texture,Context context,String modelName) {
		super(texture);				
		...
		//load texture from file.tex
		is = null;
		ByteBuffer pointVTB = null;
		try {
		        is = assManager.open(modelName+".tex");
		        pointVTB = readToByteBuffer(is);
		...
		indexCount=(int) (textureCoordBuffer.remaining()*0.5);
		
		textureCoordBuffer.position(0);
		
		//load normals from file.nrm
		is = null;
		ByteBuffer pointVNB = null;
		try {
		        is = assManager.open(modelName+".nrm");
		        pointVNB = readToByteBuffer(is);
		...
		normalBuffer=pointVNB.asFloatBuffer();		
		normalBuffer.position(0);
		
		
	
/*		is = null;
		ByteBuffer pointIBB = null;		
		try {
			        is = assManager.open(modelName+".idx");
			        pointIBB = readToByteBuffer(is);
	...
	public void draw(float[] viewMatrix,float[] projectionMatrix, float timer,float[] lightPos,float luminance) {		     	                       
		 ...
		 GLES20.glVertexAttribPointer(aNormalHandler, 3, GLES20.GL_FLOAT, false, 0, normalBuffer);
		 GLES20.glEnableVertexAttribArray(aNormalHandler);	
		 
		 GLES20.glVertexAttribPointer(aTextureCoordHolder, 2, GLES20.GL_FLOAT, false, 0, textureCoordBuffer);
